...
X=np.loadtxt("/data/ted/COAD/eso.mut.txt")
S=X.shape[0];#number of samples, from input
X=X.reshape((S,N,3))

Genome_BG=np.loadtxt("/data/ted/COAD/eso.wt.txt")
...
for iter in range(20):
	...
	X[0:10,:,:]=X_truth;
	X[10:,:,:]=X_rest[iter*10:iter*10+10,:,:]
	test_X=np.random.binomial(X.astype(int),1-p)
	...
	np.savetxt("/data/ted/COAD/eso/ReLU/Feature"+str(iter)+".txt",Feature, fmt='%s',delimiter='\t')
	np.savetxt("/data/ted/COAD/eso/ReLU/Matrix"+str(iter)+".txt",Matrix, fmt='%s',delimiter='\t')
	np.savetxt("/data/ted/COAD/eso/ReLU/P"+str(iter)+".txt",P, fmt='%.18e',delimiter='\t')
	...
	np.savetxt("/data/ted/COAD/eso/ReLU/conv"+str(iter)+".txt",conv_all, fmt='%.18e',delimiter='\t')
